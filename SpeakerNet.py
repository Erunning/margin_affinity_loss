...
from loss.mal import marginproto1


class SpeakerNet(nn.Module):

    def __init__(self, max_frames, lr = 0.0001, margin = 1, scale = 1, hard_rank = 0, hard_prob = 0, model="alexnet50", nOut = 512, nSpeakers = 1000, optimizer = 'adam', encoder = 'SAP', normalize = True, trainfunc='contrastive', **kwargs):
        super(SpeakerNet, self).__init__();

        argsdict = {'nOut': nOut, 'encoder_type':encoder}
        ...
        if trainfunc == 'angleproto':
        ...
        elif trainfunc == 'marginproto1':
            self.__L__ = marginproto1(s=scale, m=margin).cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'marginproto2':
            self.__L__ = marginproto2().cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'marginproto3':
            self.__L__ = marginproto3().cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'marginangle':
            self.__L__ = marginangle().cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'ge2e':
            ...
            self.__train_normalize__    = True
            self.__test_normalize__     = True
        elif trainfunc == 'marginaffinity':
            self.__L__ = marginaffinity().cuda()
            self.__train_normalize__    = True
            self.__test_normalize__     = True
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000, feat_dir='', test_path='', num_eval=10):
        ...
        for idx, file in enumerate(setfiles):
            ...
            inp1 = loadWAV(os.path.join(test_path,file), 400, evalmode=True, num_eval=num_eval).cuda()
