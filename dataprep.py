...
parser.add_argument('--save_path', 	type=str, default="/data/dataset", help='Target directory');
...
parser.add_argument('--convert',  default='True',  dest='convert',  action='store_true', help='Enable convert')
...
def convert(args):
	...
	folders = glob.glob('%s/vox2/dev/aac/*/*/'%args.save_path)
	files 	= glob.glob('%s/vox2/dev/aac/*/*/*.m4a'%args.save_path)
	files.sort()
