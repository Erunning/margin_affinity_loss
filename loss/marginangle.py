...
class marginangle(nn.Module):

    def __init__(self, init_w=10.0, init_b=-5.0):
        super(marginangle, self).__init__()
        self.w = nn.Parameter(torch.tensor(init_w))
        self.b = nn.Parameter(torch.tensor(init_b))
        self.m = 0.1
        self.criterion  = torch.nn.CrossEntropyLoss()

        print('Initialised marginangle')
    ...
    def forward(self, x, label=None):
        ...
        aa = out_positive.unsqueeze(-1).expand(-1,-1,stepsize)
        bb = out_anchor.unsqueeze(-1).expand(-1,-1,stepsize).transpose(0,2)
        cos_sim_matrix  = F.cosine_similarity(aa, bb)
