...
class ResNetSE(nn.Module):
    def __init__(self, block, layers, num_filters, nOut, encoder_type, **kwargs):
        ...
        if self.encoder_type == "AVGSTD":
            out_dim = num_filters[3] * block.expansion * 2
        if self.encoder_type == "AVG":
            out_dim = num_filters[3]     
        if self.encoder_type == "MA":
            multihead_attn = nn.MultiheadAttention(128, 8)
            out_dim = num_filters[3] 
    ...
    def forward(self, x):
        ...
        if self.encoder_type == "MA":
            x = x.permute(0, 2, 1, 3)
            x = x.squeeze(dim=1).permute(2, 0, 1)  # L * batch * D
...
def ResNetSE34L(nOut, encoder_type, **kwargs):
    # Number of filters
    num_filters = [16, 32, 64, 128]
    model = ResNetSE(SEBasicBlock, [3, 4, 6, 3], num_filters, nOut, encoder_type, **kwargs)
    return model
